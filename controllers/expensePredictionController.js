...
exports.predictCropProfitability = async (req, res) => {
    try {
        ...
        if (useSoilData === 'true' && fieldId) {
            const soil = await SoilAnalysis.findOne({ farmerId: userId, fieldId })
                .sort({ dateTested: -1 });
            ...
            if (soil) {
                ...
                if (soil.potassium === 'Low') currentRates.fertilizers *= 1.10;
            }
        }
        ...
        if (farmer && farmer.coordinates && farmer.coordinates.lng && farmer.coordinates.lat) {
        ...
        const expectedRevenueMarket = regionalPrice 
            ...
            : (expectedRevenueMSP ? expectedRevenueMSP * 1.15 : null);
