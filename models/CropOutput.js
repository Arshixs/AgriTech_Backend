...
// // ============================================
// models/CropOutput.js - UPDATED WITH SEPARATE STATUS FIELDS
const mongoose = require('mongoose');
...
const cropOutputSchema = new mongoose.Schema({
  ...
  // QUALITY STATUS - Separate from sale status
  qualityStatus: {
    type: String,
  ...
  // SALE STATUS - Separate from quality status
  status: {
    type: String,
    enum: ['available', 'listed-for-sale', 'sold', 'reserved'],
    default: 'available',
  },
...
// Index for efficient queries
cropOutputSchema.index({ farmerId: 1, status: 1 });
cropOutputSchema.index({ farmerId: 1, qualityStatus: 1 });
cropOutputSchema.index({ cropId: 1 });
